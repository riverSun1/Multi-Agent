...
그렇기 때문에 시나리오가 많으면 usecase가 많을 수 있습니다.
...
이검ㅅ은 반대로 우리 측에서 API를 요청하는 상황이 되는데
기존 layered 구성에서는 이 코드를 작성하기가 상당히 난감했음.
어디에 배치하는 것이 가장 좋을까?
응? api만들고 보니 api에 잔뜩 쌓여서 구별이 안되기 시작하네ㅠ
위와 같은 상황들이 발생할 수 있다는 점이 있었습니다.

특정 도메인에서 외부 api가 필요하다면 그냥 싹다 output으로 내보내면 됩니다.
그러니 위의 api 패키지에 싹 다 때려넣고 고통 받을 일이 없다는 뜻입니다.
```
...
어찌보면 외부 프레임 워크라고 봐도 괜찮을 것 같습니다.
```

```
벌집처럼 domain을 중간에 두고 참조하는 느낌 <- 정.답
